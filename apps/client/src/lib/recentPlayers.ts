...
const DEFAULT_PLAYERS: RecentPlayer[] = [
...
function loadRecent(serverId: string): RecentPlayer[] {
  try {
...
export function getRecentPlayers(serverId: string): RecentPlayer[] {
  return loadRecent(serverId);
}

export function addRecentPlayer(username: string, skinColor: string, serverId: string): void {
  try {
