...
export function useMultiplayer({ token, localUserId, positionRef, directionRef, enabled = true, onForcedLogout }) {
    const remotePlayersMapRef = useRef(new Map());
    const remotePlayerIdsRef = useRef([]);
    ...
    const pushSocketLog = useCallback((kind, message) => {
        const nextId = ++logIdRef.current;
        setSocketLogs((prev) => [...prev, { id: nextId, timestamp: Date.now(), kind, message }].slice(-socketLogMaxRef.current));
    }, []);
        ...
        const connect = () => {
            ...
            ws.onmessage = (event) => {
                ...
                try {
                    ...
                    if (msg.type !== 'presence_update' || !Array.isArray(msg.players))
                        return;
                    setReceivedUpdates((n) => n + 1);
                    ...
                    const idsChanged = newIds.length !== prevIds.length || newIds.some((id, i) => id !== prevIds[i]);
