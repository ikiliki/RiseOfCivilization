...
function classifyPhase(title, currentPhase) {
    const normalized = `${title} ${currentPhase}`.toLowerCase();
    if (/phase 3|operations|hardening|runbook|observability|integration test|contract test|docs portal|quality|debug/i.test(normalized)) {
        return 'Phase 3';
    }
...
export function extractRoadmapPhases(roadmapMarkdown, currentPhase) {
    ...
    return matches.map((match, index) => {
        ...
        else if (normalizedTitle.includes('(current)') ||
...
export function getPhaseOptions(items, phases) {
    return [...new Set([...phases.map((p) => p.title), ...items.map((item) => item.phase)])];
}
export function filterWorkItems(items, statusFilter, phaseFilter) {
